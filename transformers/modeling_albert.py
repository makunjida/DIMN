...
from utils_multiple_choice import prn_obj
...
ALBERT_PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    'albert-large-v2': "{path}/albert-large-v2-pytorch_model.bin",
...
class AlbertDIMNForMultipleChoice(AlbertPreTrainedModel):

    def __init__(self, config):
        super(AlbertDIMNForMultipleChoice, self).__init__(config)

        self.albert = AlbertModel(config)
        self.classifier_2 = nn.Linear(2*config.hidden_size, 1)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.co_att= BertCoAttention(config)
        self.init_weights()
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        context_sequence_output, query_sequence_output, context_attention_mask, query_attention_mask = \
            split_context_query(sequence_output, pq_end_pos)
        
        for _ in range(0,iter): 
            cq_biatt_output = self.co_att(context_sequence_output, query_sequence_output, context_attention_mask)
            qc_biatt_output = self.co_att(query_sequence_output, context_sequence_output, query_attention_mask)
...
class BertCoAttention(nn.Module):
    ...
    def forward(self, context_states, query_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        #* mixed_query_layer     [4, 512, 768]
        query_layer = self.transpose_for_scores(mixed_query_layer)
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)
        #* [4, 12, 512, 64]000 c 3                

        #*  key_layer.transpose(-1, -2)     [4, 12, 64, 512]
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        #* [4, 12, 512, 512]
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
